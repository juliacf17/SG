...
class MyScene extends THREE.Scene {
  ...
  constructor (myCanvas) { 
    ...
    // this aqui es el nodo raíz del grafo
    this.createLights ();
  ...
  createGUI () {
    ...
    folder.add (this.guiControls, 'lightPower', 0, 200, 10) // Las opciones son: el objeto de control, el atributo a controlar, y el intervalo
      .name('Luz puntual : ') // Nombre que aparecerá en la interfaz
      .onChange ( (value) => this.setLightPower(value) ); // onChange --> solo cuando el usuario cambia el valor
    ...
    folder.add (this.guiControls, 'axisOnOff')  // Las opciones son: el objeto de control, el atributo a controlar
      .name ('Mostrar ejes : ')
      .onChange ( (value) => this.setAxisVisible (value) ); // onChange --> solo cuando el usuario cambia el valor
  ...
  createLights () {
    ...
    this.pointLight = new THREE.SpotLight( 0xffffff ); // Luz blanca
    this.pointLight.power = this.guiControls.lightPower; // La potencia
    this.pointLight.position.set( 2, 3, 1 ); // La posición
    this.add (this.pointLight); // Y se añade a la escena
  }
...
$(function () {
  ...
  var scene = new MyScene("#WebGL-output"); // WebGL-output es el ID del lienzo
